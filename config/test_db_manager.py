    ...
    def test_engine(self) -> Engine:
        ...
        if self._test_engine is None:
            self._test_engine = create_engine(
                self.settings.test_database_url, **self.settings.test_engine_config
            )
    ...
    def session_factory(self) -> sessionmaker:
        ...
        if self._session_factory is None:
            self._session_factory = sessionmaker(
                bind=self.test_engine, autocommit=False, autoflush=False
            )
        ...
        try:
            with self.test_engine.connect() as conn:
                if preserve_schema:
                    ...
                    logger.info(
                        f"🧹 Cleaned test data from {len(metadata.tables)} tables"
                    )
                else:
                    # Drop all tables
                    metadata = MetaData()

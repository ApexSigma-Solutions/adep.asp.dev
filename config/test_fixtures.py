...
from .test_db_manager import get_test_db_manager, test_database_session, cleanup_test_environment
...
def isolated_db_session(test_db_manager) -> Generator[Session, None, None]:
    ...
    """
    test_id = f"pytest_{pytest.current_test_id()}" if hasattr(pytest, 'current_test_id') else None
...
def test_engine(test_settings):
    ...
    engine = create_engine(
        ...
        **test_settings.test_engine_config
    )
    yield engine
    engine.dispose()
...
def setup_test_environment(test_settings):
    ...
    logger.info(f"🧪 Test environment initialized - Session: {test_settings.TEST_SESSION_ID}")
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        # Mark database tests
        if any(fixture in item.fixturenames for fixture in ["isolated_db_session", "test_engine", "clean_database"]):
...
class ServiceTestHelper:
    ...
    def wait_for_service_ready(self, service_name: str, health_check_url: str, timeout: int = None) -> bool:

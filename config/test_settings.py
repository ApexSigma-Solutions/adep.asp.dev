...
class TestSettings(BaseSettings):
    ...
    TEST_SESSION_ID: str = Field(
        ...
        description="Unique test session identifier",
    )

    # Test database configuration - isolated from production
    TEST_POSTGRES_DB: str = Field(
        default="apexsigma_test_db", description="Test database name"
    )
    TEST_POSTGRES_HOST: str = Field(
        default="apexsigma_postgres", description="Test database host"
    )
    TEST_POSTGRES_PORT: int = Field(default=5432, description="Test database port")
    TEST_POSTGRES_USER: str = Field(
        default="apexsigma_test_user", description="Test database user"
    )
    TEST_POSTGRES_PASSWORD: str = Field(
        default="test_password_secure", description="Test database password"
    )
    ...
    TEST_MAX_OVERFLOW: int = Field(
        default=10, description="Test max overflow connections"
    )
    TEST_POOL_TIMEOUT: int = Field(
        default=10, description="Test pool timeout in seconds"
    )
    TEST_POOL_RECYCLE: int = Field(
        default=300, description="Test connection recycle time in seconds"
    )

    # Test isolation configuration
    USE_TRANSACTIONS: bool = Field(
        default=True, description="Use transactions for test isolation"
    )
    AUTO_ROLLBACK: bool = Field(
        default=True, description="Automatically rollback test transactions"
    )
    CLEAN_BETWEEN_TESTS: bool = Field(
        default=True, description="Clean data between tests"
    )

    # Test data management
    TEST_DATA_RETENTION: int = Field(
        default=0, description="Hours to retain test data (0 = immediate cleanup)"
    )
    PRESERVE_TEST_LOGS: bool = Field(
        default=False, description="Preserve test execution logs"
    )

    # Service synchronization for tests
    SERVICE_STARTUP_TIMEOUT: int = Field(
        default=30, description="Service startup timeout in seconds"
    )
    HEALTH_CHECK_INTERVAL: int = Field(
        default=1, description="Health check interval in seconds"
    )
    HEALTH_CHECK_RETRIES: int = Field(
        default=10, description="Number of health check retries"
    )
    ...
    TEST_TIMEOUT: int = Field(
        default=60, description="Individual test timeout in seconds"
    )
    FAST_TEST_MODE: bool = Field(
        default=False, description="Enable fast test mode (reduced waits)"
    )
...
def get_test_settings() -> TestSettings:
    ...
    if is_testing:
        ...
        print(
            f"   Connection pool: {settings.TEST_POOL_SIZE} base, {settings.TEST_MAX_OVERFLOW} overflow"
        )

...
class MemosServiceTestUtils(ServiceTestBase):
    ...
    def create_test_tool_registry_entry(
...
class ToolsServiceTestUtils(ServiceTestBase):
    ...
    def create_test_execution_log(
        self, session: Session, tool_id: str, **kwargs
    ) -> Dict[str, Any]:
...
class IngestLLMServiceTestUtils(ServiceTestBase):
    ...
    def create_test_embedding(
        self, session: Session, document_id: str, **kwargs
    ) -> Dict[str, Any]:
...
class IntegrationTestUtils:
    ...
    def isolated_ecosystem_test(self, test_id: Optional[str] = None):
        ...
        try:
            for service_name, service_utils in self.services.items():
                session_manager = service_utils.db_manager.isolated_session(
                    f"{ecosystem_test_id}_{service_name}"
                )
            ...
            for service_name, session in sessions.items():
                try:
                    session_manager = self.services[
                        service_name
                    ].db_manager.isolated_session()
